Vagrant.configure("2") do |config|
    ...
    %w{node1 node2 node3}.each_with_index do |name, i|
      config.vm.define name do |node|
        ...
        node.vm.network :private_network, ip: "192.168.60.#{i + 11}"
        node.vm.provision :shell, privileged: false, inline: <<-SHELL
  sudo /vagrant/join.sh
  echo 'Environment="KUBELET_EXTRA_ARGS=--node-ip=192.168.60.#{i + 11}"' | sudo tee -a /etc/systemd/system/kubelet.service.d/10-kubeadm.conf
  ...
  echo "192.168.60.10 master" >> /etc/hosts
  ...
  # Start cluster
  sudo kubeadm init --apiserver-advertise-address=192.168.60.10 --pod-network-cidr=10.244.0.0/16 | grep -Ei "kubeadm join" | awk '{print $0 "--discovery-token-unsafe-skip-ca-verification"}' > ${OUTPUT_FILE}
  chmod +x $OUTPUT_FILE
  ...
  # Fix kubelet IP
  echo 'Environment="KUBELET_EXTRA_ARGS=--node-ip=192.168.60.10"' | sudo tee -a /etc/systemd/system/kubelet.service.d/10-kubeadm.conf
  ...
  sudo echo "alias k=/usr/bin/kubectl" >> $HOME/.bashrc
  
  # add kubectl autocomplete
  sudo echo 'source <(kubectl completion bash)' >> $HOME/.bashrc
  sudo echo 'source <(kubectl completion bash)' >> $HOME/.bash_profile
  
  #make shell pretty
  sudo sed -i "/force_color_prompt/s/^#//g" $HOME/.bashrc
